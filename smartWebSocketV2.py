...
class SmartWebSocketV2(object):
    ...
    def _parse_binary_data(self, binary_data):
        ...
        # Divide by 10,000,000.0 for Currency (CDE_FO = 13), and by 100.0 for all other segments
        divisor = 10000000.0 if exchange_type == self.CDE_FO else 100.0
        ...
        parsed_data = {
            ...
            "last_traded_price": self._unpack_data(binary_data, 43, 51, byte_format="q")[0] / divisor
        }
        try:
            ...
            if parsed_data["subscription_mode"] in [self.QUOTE, self.SNAP_QUOTE]:
                ...
                parsed_data["average_traded_price"] = self._unpack_data(binary_data, 59, 67, byte_format="q")[0] / divisor
                ...
                parsed_data["open_price_of_the_day"] = self._unpack_data(binary_data, 91, 99, byte_format="q")[0] / divisor
                parsed_data["high_price_of_the_day"] = self._unpack_data(binary_data, 99, 107, byte_format="q")[0] / divisor
                parsed_data["low_price_of_the_day"] = self._unpack_data(binary_data, 107, 115, byte_format="q")[0] / divisor
                parsed_data["closed_price"] = self._unpack_data(binary_data, 115, 123, byte_format="q")[0] / divisor
            ...
            if parsed_data["subscription_mode"] == self.SNAP_QUOTE:
                ...
                parsed_data["upper_circuit_limit"] = self._unpack_data(binary_data, 347, 355, byte_format="q")[0] / divisor
                parsed_data["lower_circuit_limit"] = self._unpack_data(binary_data, 355, 363, byte_format="q")[0] / divisor
                parsed_data["52_week_high_price"] = self._unpack_data(binary_data, 363, 371, byte_format="q")[0] / divisor
                parsed_data["52_week_low_price"] = self._unpack_data(binary_data, 371, 379, byte_format="q")[0] / divisor
                best_5_buy_and_sell_data = self._parse_best_5_buy_and_sell_data(binary_data[147:347], divisor)
            ...
            if parsed_data["subscription_mode"] == self.DEPTH:
                ...
                depth_20_data = self._parse_depth_20_buy_and_sell_data(binary_data[depth_data_start_index:], divisor)
    ...
    def _parse_best_5_buy_and_sell_data(self, binary_data, divisor):
        ...
        for packet in best_5_buy_sell_packets:
            each_data = {
                ...
                "price": self._unpack_data(packet, 10, 18, byte_format="q")[0] / divisor,
    ...
    def _parse_depth_20_buy_and_sell_data(self, binary_data, divisor):
        depth_20_buy_data = []
        depth_20_sell_data = []
        ...
        for i in range(20):
            ...
            buy_packet_data = {
                ...
                "price": self._unpack_data(binary_data, buy_start_idx + 4, buy_start_idx + 8, byte_format="i")[0] / divisor,
            ...
            sell_packet_data = {
                ...
                "price": self._unpack_data(binary_data, sell_start_idx + 4, sell_start_idx + 8, byte_format="i")[0] / divisor,
